...
resource "harvester_virtualmachine" "primary" {
  ...
  hostname        = "${var.username}-dbprimary-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
...
resource "harvester_virtualmachine" "replica" {
  ...
  run_strategy    = "RerunOnFailure"
  hostname        = "${var.username}-dbreplica-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  reserved_memory = "100Mi"
  machine_type    = "q35"
