...
resource "harvester_virtualmachine" "vm" {
  ...
  name                 = "${var.username}-versity-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  namespace            = local.namespace
  ...
  hostname        = "${var.username}-versity-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  ...
  tags = {
    ...
    condenser_ingress_os_port = 7070
    ...
    condenser_ingress_cons_port = 7071
